...
**목표:** 단일 파일 강의 어노테이션 도구의 기능 추가·개선·버그수정을 구현→검증→UX검수
파이프라인으로 안정적으로 수행한다.

**트리거:** 어노테이션/강의 도구의 기능 추가·개선·버그수정·QA·UX 점검(및 "다시 실행/
재실행/업데이트/보완" 등 후속 요청) 시 `lecture-harness` 스킬을 사용하라.
단순 질문이나 설명 요청은 직접 응답 가능.

**구성:** 에이전트 `annotation-dev`·`canvas-qa`·`lecture-ux-reviewer`(`.claude/agents/`),
스킬 `annotation-feature-dev`·`canvas-qa`·`lecture-ux-review`·`lecture-harness`
(`.claude/skills/`). 상세는 각 파일과 오케스트레이터 스킬이 관리.

**변경 이력:**
| 날짜 | 변경 내용 | 대상 | 사유 |
|------|----------|------|------|
| 2026-06-07 | 초기 구성 (3 에이전트 + 4 스킬) | 전체 | 하네스 신규 구축 |
